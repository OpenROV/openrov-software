...
(function (exports) {
  ...
  function extend(target, src) {
    var options, name, copy, copyIsArray, clone, i = 1, length = 2, deep = true;
    ...
    if (options = src) {
      ...
      for (name in options) {
        ...
        if (deep && (typeof copy == 'object' || (copyIsArray = Object.prototype.toString.call(copy) === '[object Array]'))) {
          ...
          target[name] = extend(clone, copy);  // Don't bring in undefined values
  ...
  exports.GameController = {
    ...
    resize: function (firstTime) {
      // Scale to same size as original canvas
      this.canvas.width = this.options.canvas.scrollWidth;
      this.canvas.height = this.options.canvas.scrollHeight;
      ...
      if (this.options.canvas.style.width && this.options.canvas.style.height && this.options.canvas.style.height.indexOf('px') !== -1) {
        this.canvas.style.width = this.options.canvas.style.scrollWidth;
        this.canvas.style.height = this.options.canvas.style.scrollHeight;
    ...
    addJoystick: function (options) {
      //x, y, radius, backgroundColor, touchStart, touchEnd ) {
      ...
      if (!this.paused && !this.performanceFriendly) {
        ...
        for (var i = 0, j = this.touches.length; i < j; i++) {
          ...
          if (typeof touch === 'undefined')
            continue;
          var x = this.normalizeTouchPositionX(touch.clientX), y = this.normalizeTouchPositionY(touch.clientY);
          if (x - this.options.touchRadius > this.bound.left && x + this.options.touchRadius < this.bound.right && y - this.options.touchRadius > this.bound.top && y + this.options.touchRadius < this.bound.bottom)
