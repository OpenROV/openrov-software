...
function OpenRovViewModel() {
  ...
  self.currentCpuUsage = ko.computed(function () {
    return (self.currentRawCpuUsage() * 100).toFixed(0);
  });
  ...
  self.servoTiltStyle = ko.computed(function () {
    var angle = 90 / 500 * self.currentTiltPosition() + 90;
    //*-45;
  ...
  self.updateSettings = function (settings) {
    if ('deadzone_pos' in settings)
      self.deadzone_pos(settings.deadzone_pos);
    if ('deadzone_neg' in settings)
      self.deadzone_neg(settings.deadzone_neg);
    if ('smoothingIncriment' in settings)
      self.smoothingIncriment(settings.smoothingIncriment);
    if ('reverse_port_thruster' in settings)
      self.reversePortThruster(settings.reverse_port_thruster);
    if ('reverse_starbord_thruster' in settings)
      self.reverseStarbordThruster(settings.reverse_starbord_thruster);
    if ('reverse_lift_thruster' in settings)
      self.reverseLiftThruster(settings.reverse_lift_thruster);
    if ('googletalk_rovid' in settings)
      self.googleTalkROVid(settings.googletalk_rovid);
    if ('googletalk_rovpassword' in settings)
      self.googleTalkROVpassword(settings.googletalk_rovpassword);
    if ('googletalk_rov_pilotid' in settings)
      self.googleTalkPilotId(settings.googletalk_rov_pilotid);
  };
  ...
  self.updateStatus = function (data) {
    ...
    if ('servo' in data)
      self.currentTiltPosition(data.servo);
    if ('cpuUsage' in data)
    ...
    for (var item in self.telemetry) {
      if (self.telemetry.hasOwnProperty(item)) {
        self.rawTelemetry().push({
          key: item,
          value: self.telemetry[item]
        });
      }
    }
